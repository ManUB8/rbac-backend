...
def parse_time_dot(value):
    ...
    if isinstance(value, str):
        ...
        if len(parts) != 2:
            raise ValueError("รูปแบบเวลาต้องเป็น HH.MM เช่น 14.00")
...
def validate_check_type(value: str):
    ...
    if value not in allowed:
        raise ValueError(
            "check_type ต้องเป็น checkin_only, checkout_only หรือ checkin_checkout"
        )
...
class ActivityCreateRequest(BaseModel):
    ...
    def validate_check_type_time(self):
        if self.check_type == "checkin_only":
            if self.checkin_open_time is None or self.checkin_close_time is None:
                raise ValueError(
                    "กิจกรรมแบบเช็คอิน ต้องกรอกเวลาเปิดและปิดเช็คอิน"
                )

            self.checkout_open_time = None
            self.checkout_close_time = None

        elif self.check_type == "checkout_only":
            if self.checkout_open_time is None or self.checkout_close_time is None:
                raise ValueError(
                    "กิจกรรมแบบเช็คเอาท์ ต้องกรอกเวลาเปิดและปิดเช็คเอาท์"
                )

            self.checkin_open_time = None
            self.checkin_close_time = None

        elif self.check_type == "checkin_checkout":
            if (
                self.checkin_open_time is None
                or self.checkin_close_time is None
                or self.checkout_open_time is None
                or self.checkout_close_time is None
            ):
                raise ValueError(
                    "กิจกรรมแบบเช็คอินและเช็คเอาท์ ต้องกรอกเวลาทั้งสองส่วน"
                )
